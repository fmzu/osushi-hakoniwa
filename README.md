...
`index.html` をブラウザで開くだけ。住人は サーモン・まぐろ・たまご・えび・いか・いくら（軍艦） の6種で、時間経過でもふらっと増えます。未発見の子をタップすると「はっけん！」で図鑑に登録され、発見状況はlocalStorageに保存されます。図鑑はプロフィール帳風で、なまえ・せいかくの2項目。せいかく（のんびりや・せっかち・あまえんぼ・はずかしがりや・ねぼすけ・こうきしんおうせい）は歩くテンポ・休憩率・♡率・Uターン率など実際の挙動パラメータに直結しています。
